...
function _family_spec(
    ...
    return DistributionFamilySpec(
        ...
        mixture_component, backend, device,
    )
end
...
const DISTRIBUTION_FAMILY_TABLE = (
    ...
    _family_spec(:lognormal; latent=:scalar, transform=:log, backend=true, device=true),
    _family_spec(:laplace; latent=:scalar, transform=:identity, mixture_component=true, backend=true, device=true),
    _family_spec(:exponential; broadcastable=true, latent=:scalar, transform=:log, backend=true, device=true),
    _family_spec(:gamma; latent=:scalar, transform=:log, backend=true, device=true),
    _family_spec(:inversegamma; latent=:scalar, transform=:log, backend=true, device=true),
    _family_spec(:weibull; latent=:scalar, transform=:log, backend=true, device=true),
    _family_spec(:beta; latent=:scalar, transform=:logit, backend=true, device=true),
    _family_spec(:dirichlet; latent=:conditional, transform=:simplex, backend=true, device=true),
    _family_spec(:bernoulli; broadcastable=true, backend=true, device=true),
    _family_spec(:bernoullilogit; broadcastable=true, backend=true, device=true),
    _family_spec(:binomial; backend=true, device=true),
    _family_spec(:betabinomial; backend=true),
    _family_spec(:multinomial),
    _family_spec(:discreteuniform; backend=true),
    _family_spec(:geometric; backend=true, device=true),
    _family_spec(:negativebinomial; backend=true, device=true),
    _family_spec(:poisson; broadcastable=true, backend=true, device=true),
    _family_spec(
        :studentt;
        broadcastable=true,
        latent=:scalar,
    ...
    _family_spec(:cauchy; latent=:scalar, transform=:identity, backend=true, device=true),
    _family_spec(:halfnormal; latent=:scalar, transform=:log, backend=true, device=true),
    _family_spec(:halfcauchy; latent=:scalar, transform=:log, backend=true, device=true),
    _family_spec(:uniform; latent=:conditional, transform=:bounded, backend=true),
    _family_spec(:logistic; latent=:scalar, transform=:identity, backend=true, device=true),
    _family_spec(:gumbel; latent=:scalar, transform=:identity, backend=true, device=true),
    _family_spec(:pareto; latent=:conditional, transform=:lowerbounded, backend=true),
    _family_spec(:frechet; latent=:scalar, transform=:log, backend=true, device=true),
    _family_spec(:rayleigh; latent=:scalar, transform=:log, backend=true, device=true),
    _family_spec(:inversegaussian; latent=:scalar, transform=:log, backend=true, device=true),
)
