...
function batched_meads(
    ...
    while true
        ...
        if !_init_is_redrawable(initial_params) || init_attempt >= init_max_retries
            ...
            throw(
                ArgumentError(
                    "initial batched MEADS parameters produced a non-finite unconstrained logjoint or gradient in $(length(bad_columns)) of $num_chains chain(s)$retried; try init=:uniform or supply finite initial_params",
                ),
            )
        end
        init_attempt += 1
