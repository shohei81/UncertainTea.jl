...
function batched_nuts(
    ...
    while true
        ...
        if !_init_is_redrawable(initial_params) || init_attempt >= init_max_retries
            ...
            throw(
                ArgumentError(
                    "initial batched NUTS parameters produced a non-finite unconstrained logjoint or gradient in $(length(bad_columns)) of $num_chains chain(s)$retried; try init_strategy=:uniform or supply finite initial_params",
                ),
            )
        end
        init_attempt += 1
